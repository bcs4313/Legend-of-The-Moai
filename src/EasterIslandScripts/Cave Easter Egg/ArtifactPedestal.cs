...
namespace EasterIsland.src.EasterIslandScripts.Cave_Easter_Egg
{
    ...
    public class ArtifactPedestal : NetworkBehaviour
    {
        bool activated = false;
        public GameObject artifactPlaceholder;  // visual, starts as disabled

        public void Start()
        {
            if (RoundManager.Instance.IsHost)
            {
                artifactPlaceholder.SetActive(false);
            }
        }
        ...
        public void interact(PlayerControllerB ply)
        {
            ...
            {
                Debug.Log("Inserting Artifact Into Pedestal");
                if (RoundManager.Instance.IsHost)
                {
        ...
        public void setActivatedServerRpc()
        {
            setActivatedClientRpc();
        }
        ...
        {
            activated = true;
            artifactPlaceholder.SetActive(true);
        }
        ...
        public void takeItemClientRpc(ulong playerid, ulong itemid)
        {
            ...
            if (!targetPlayer)
            {
                UnityEngine.Debug.LogError("Radar Pedestal: Could not find target player for item removal! Cancelling item removal.");
                return;
            }
            ...
            for (int i = 0; i < inventory.Length; i++)
            {
                ...
                if (obj.NetworkObjectId == itemid)
                {
                    if (!obj.isPocketed)
                    {
                        UnityEngine.Debug.Log("Radar Pedestal: Despawned Held Object (Radar)");
                        targetPlayer.DespawnHeldObject();
                    }
                    else
                    {
                        UnityEngine.Debug.Log("Radar Pedestal: Despawned Object in Slot (Radar)");
            ...
            UnityEngine.Debug.LogError("Radar Pedestal: Could not find target item for removal (player was found)! Cancelling item removal.");
        }
