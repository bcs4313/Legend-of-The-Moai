...
namespace MoaiEnemy.src.MoaiNormal
{
    public class PlasmaBall : NetworkBehaviour
    {
        ...
        protected float selfCollisionThreshold = 0.25f;
        ...
        void OnCollisionEnter(UnityEngine.Collision collision)
        {
            ...
            if(Time.time - creationTime < selfCollisionThreshold)  // prevents self collision
            {
                return;
            }
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            if (Time.time - creationTime < selfCollisionThreshold) // Prevents self-trigger interaction
            {
                return;
            }
            ...
            if (hit)
            {
                ...
                Debug.Log($"PlasmaBall triggered by: {hit.name}");
                ...
                {
                    // Damage enemy AI
                    Debug.Log($"PlasmaBall triggered enemyAI: {enemyAI.name}");
                    enemyAI.HitEnemy(8, null, true);
                }
        ...
        void spawnExplosionClientRpc()
        {
            ...
            laserHit.Play();
            Landmine.SpawnExplosion(transform.position, true, 9f, 12f, 30, 40f, null, true);

            // to trigger company damage
            if(RoundManager.Instance.IsHost)
            {
                var fightScripts = GameObject.FindObjectsOfType<CompanyFightScript>();

                foreach(var script in fightScripts)
                {
                    // check how close the explosion was to the company hitbox
                    var loc = script.companyHitBox.transform.position;

                    if (UnityEngine.Vector3.Distance(this.transform.position, loc) <= 12 && !hasDamagedBoss)
                    {
                        hasDamagedBoss = true;
                        if (CompanyFightScript.hostile == false)
                        {
                            script.beginFight();
                        }
                        else
